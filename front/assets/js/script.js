jQuery(function ($) {
    ...
    function putInCache(key,data,isJSON) { if(isJSON===true) { storage.setItem(key,JSON.stringify(data)); } else { storage.setItem(key,data); } }
    function getFromCache(key,isJSON) { if(isJSON===true) { return JSON.parse(storage.getItem(key)); } else { return storage.getItem(key); }}
